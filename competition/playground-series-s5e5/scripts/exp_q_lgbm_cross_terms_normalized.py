# Script for Experiment Q: LightGBM with Cross-Term Features and Normalized Target
# MODIFIED for Playground Series S5E5: Predict Calorie Expenditure

"""
Extended Description:
This script combines the approaches from:
1. Experiment P: Cross-term features (multiplication between numerical variables)
2. Experiment K: Target normalization by Duration (calories per minute)
...
- Transforming predictions back to the original scale by multiplying by Duration after expm1
...
Corresponds to experiment 'exp_q_lgbm_cross_terms_normalized.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_q_lgbm_cross_terms_normalized.py"
...
def main():
    ...
    try:
        ...
        for fold, (train_idx, valid_idx) in enumerate(kf.split(X, y)):
            ...
            fold_oof_preds_log = model.predict(X_valid_fold)
            fold_test_preds_log = model.predict(X_test)
        ...
        final_test_preds_original_scale = np.expm1(final_test_preds_log) * test_duration
        
        # Clip final predictions
        final_test_preds_original_scale = np.clip(final_test_preds_original_scale, 1, 314)
        oof_preds_original_scale = np.clip(oof_preds, 1, 314)
        ...
        if exp_mask.sum() > 0:
            ...
            experiments_df.loc[idx_to_update, 'why'] = 'Combine cross-term features with target normalization.'
            experiments_df.loc[idx_to_update, 'what'] = 'Train LightGBM with cross-term features using normalized target (calories per minute).'
            experiments_df.loc[idx_to_update, 'how'] = 'Create cross-terms, normalize target by Duration, 5-fold CV, transform predictions back: expm1(pred)*Duration.'
            experiments_df.loc[idx_to_update, 'experiment_type'] = 'single_model'
            experiments_df.loc[idx_to_update, 'base_experiment'] = 'exp_p_lgbm_cross_terms.py'
            experiments_df.loc[idx_to_update, 'new_feature'] = 'Target normalization (calories per minute)'
            ...
            new_row = pd.DataFrame([{
                ...
                'why': 'Combine cross-term features with target normalization.',
                'what': 'Train LightGBM with cross-term features using normalized target (calories per minute).',
                'how': 'Create cross-terms, normalize target by Duration, 5-fold CV, transform predictions back: expm1(pred)*Duration.',
                'cv_score': overall_cv_score,
                'lb_score': None,
                'status': 'done',
                'experiment_type': 'single_model',
                'base_experiment': 'exp_p_lgbm_cross_terms.py',
                'new_feature': 'Target normalization (calories per minute)'
            }])
