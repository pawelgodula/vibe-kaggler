# Script for Experiment H: LightGBM with Enhanced Interaction Features (No Target Leakage)
# MODIFIED for Playground Series S5E5: Predict Calorie Expenditure

"""
Extended Description:
This script is based on Experiment F but removes the target leakage:
- Removes any interaction features that use the target variable (Calories)
- Still includes other enhanced interaction features:
...
- Enhanced interaction features (without using target variable)
...
Corresponds to experiment 'exp_h_lgbm_no_leakage.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_h_lgbm_no_leakage.py"
N_FOLDS = 7
SEED = 42
TARGET_COL = "Calories"
...
# Enhanced Interaction Features - FIXED to remove target leakage
ENHANCED_INTERACTION_CONFIG = [
    ...
    {'features': ('Age', 'Heart_Rate'), 'operation': 'multiply'},
    {'features': ('Body_Temp', 'Heart_Rate'), 'operation': 'multiply'},
    ...
    {'features': ('Heart_Rate', 'Age'), 'operation': 'divide', 'fill_value': 0.0},
...
def main():
    ...
    try:
        ...
        if exp_mask.sum() > 0:
            ...
            experiments_df.loc[idx_to_update, 'why'] = 'Fix target leakage in exp_f by removing Calories-based features.'
            experiments_df.loc[idx_to_update, 'what'] = 'Train LGBM with enhanced interaction features without using target variable.'
            experiments_df.loc[idx_to_update, 'how'] = 'Remove Calories/Duration and Calories/Weight interaction features, full data, log1p target, LGBM, KFold(7), expm1 preds.'
            logger.info(f"Updated existing experiment '{EXPERIMENT_NAME}' with CV score: {overall_cv_score:.5f}")
             ...
             new_row = pd.DataFrame([{
                 ...
                 'why': 'Fix target leakage in exp_f by removing Calories-based features.',
                 'what': 'Train LGBM with enhanced interaction features without using target variable.',
                 'how': 'Remove Calories/Duration and Calories/Weight interaction features, full data, log1p target, LGBM, KFold(7), expm1 preds.',
