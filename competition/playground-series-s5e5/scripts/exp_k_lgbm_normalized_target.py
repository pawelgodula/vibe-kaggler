# Script for Experiment K: LightGBM with Count Encoding and Target Normalization 
# MODIFIED for Playground Series S5E5: Predict Calorie Expenditure

"""
Extended Description:
This script is based on Experiment J (LightGBM with Count Encoding using Test Data) but adds 
an important modification to the target variable:
1. Normalizes the target variable (Calories) by dividing by Duration, effectively predicting 
   calories burned per minute rather than total calories
2. After prediction, transforms predictions back to the original scale by multiplying by Duration
...
Corresponds to experiment 'exp_k_lgbm_normalized_target.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_k_lgbm_normalized_target.py"
N_FOLDS = 7
SEED = 42
TARGET_COL = "Calories"
...
def main():
    ...
    try:
        ...
        if exp_mask.sum() > 0:
            ...
            experiments_df.loc[idx_to_update, 'why'] = 'Test target normalization by duration to predict calories per minute.'
            experiments_df.loc[idx_to_update, 'what'] = 'Train LGBM using count encoding and target normalization by duration.'
            experiments_df.loc[idx_to_update, 'how'] = 'Normalize target by dividing Calories by Duration, apply count encoding using TEST data, KFold(7) LGBM, generate feature importance.'
            experiments_df.loc[idx_to_update, 'experiment_type'] = 'single_model'
            experiments_df.loc[idx_to_update, 'base_experiment'] = 'exp_j_lgbm_count_encoding_test.py'
            experiments_df.loc[idx_to_update, 'new_feature'] = 'Target normalization by duration (calories per minute)'
            logger.info(f"Updated existing experiment '{EXPERIMENT_NAME}' with CV score: {overall_cv_score:.5f}")
             ...
             new_row = pd.DataFrame([{
                 ...
                 'why': 'Test target normalization by duration to predict calories per minute.',
                 'what': 'Train LGBM using count encoding and target normalization by duration.',
                 'how': 'Normalize target by dividing Calories by Duration, apply count encoding using TEST data, KFold(7) LGBM, generate feature importance.',
                 'cv_score': overall_cv_score,
                 'lb_score': None,
                 'status': 'done',
                 'experiment_type': 'single_model',
                 'base_experiment': 'exp_j_lgbm_count_encoding_test.py',
                 'new_feature': 'Target normalization by duration (calories per minute)'
             }])
