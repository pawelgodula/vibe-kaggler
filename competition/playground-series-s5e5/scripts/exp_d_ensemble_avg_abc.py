# Script for Experiment D: Ensemble Average of Experiments A, B, C
# MODIFIED for Playground Series S5E5: Predict Calorie Expenditure

"""
Extended Description:
This script creates an ensemble of three previous experiments by simple averaging:
- exp_a_lgbm_combinations_te.py (10% data, log1p transform)
...
Corresponds to experiment 'exp_d_ensemble_avg_abc.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_d_ensemble_avg_abc.py"
SEED = 42
TARGET_COL = "Calories"
...
# Input Predictions Paths
EXPERIMENT_DETAILS = [
    {
        'name': 'exp_a_lgbm_combinations_te',
        'oof_path': OOF_PREDS_DIR / 'exp_a_lgbm_combinations_te_oof.npy',
        'test_path': TEST_PREDS_DIR / 'exp_a_lgbm_combinations_te_test_avg.npy',
        'weight': 1.0
    },
    {
    ...
    try:
        ...
        for exp in EXPERIMENT_DETAILS:
            try:
                ...
                # Check if the OOF predictions from exp_a need to be aligned (since it used 10% sampling)
                if exp['name'] == 'exp_a_lgbm_combinations_te':
                    logger.info(f"Handling OOF predictions from {exp['name']} separately (10% sampling)")
                    # We can't use these directly in the ensemble because they're only for 10% of the data
                    # For now, we'll skip them in the OOF ensemble but use them for test predictions
                    continue
        ...
        if len(oof_preds_list) < 2:
            ...
            for i, preds in enumerate(oof_preds_list):
                logger.info(f"Adding {EXPERIMENT_DETAILS[i+1]['name']} predictions to ensemble...")  # i+1 to skip exp_a
                oof_ensemble_preds += preds * EXPERIMENT_DETAILS[i+1]['weight']
            
            # Normalize by sum of weights
            sum_weights = sum(EXPERIMENT_DETAILS[i+1]['weight'] for i in range(len(oof_preds_list)))
        ...
        # Evaluate the ensemble performance using exp_b and exp_c OOF predictions
        logger.info("Evaluating ensemble performance...")
        # Load the actual target values from the full train set to match exp_b and exp_c
        y_true = df_train[TARGET_COL].to_numpy()
        
        # Make sure the lengths match (they should for exp_b and exp_c since they used 100% data)
        if len(y_true) != len(oof_ensemble_preds):
            ...
            # If they don't match, we might need to sample y_true or adjust in some way
            # This is just a basic implementation - in a real scenario we'd need to handle more carefully
        ...
        try:
            ...
            if exp_mask.sum() > 0:
                ...
                experiments_df.loc[idx_to_update, 'why'] = 'Ensemble the predictions from exp_a, exp_b, and exp_c to improve performance.'
                experiments_df.loc[idx_to_update, 'what'] = 'Simple average ensemble of three LGBM approaches.'
                experiments_df.loc[idx_to_update, 'how'] = 'Average OOF/test predictions from exp_a (10%), exp_b (100%), and exp_c (normalized target).'
                ...
                new_row = pd.DataFrame([{
                    ...
                    'why': 'Ensemble the predictions from exp_a, exp_b, and exp_c to improve performance.',
                    'what': 'Simple average ensemble of three LGBM approaches.',
                    'how': 'Average OOF/test predictions from exp_a (10%), exp_b (100%), and exp_c (normalized target).',
