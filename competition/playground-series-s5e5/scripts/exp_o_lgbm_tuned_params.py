# Script for Experiment O: LightGBM with Count Encoding and Tuned Parameters
# MODIFIED for Playground Series S5E5: Predict Calorie Expenditure

"""
Extended Description:
This script is based on Experiment K (LightGBM with Count Encoding) but with:
1. Modified LightGBM parameters (using n_estimators=2000, learning_rate=0.02, max_depth=10, etc.)
2. No target normalization (using log1p(Calories) directly instead of log1p(Calories/Duration))
...
- Training a LightGBM model using KFold cross-validation (7 folds) with tuned parameters
...
Corresponds to experiment 'exp_o_lgbm_tuned_params.py' in experiments.csv.
"""
...
try:
    ...
    # Replace target encoding with count encoding
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_o_lgbm_tuned_params.py"
N_FOLDS = 7
SEED = 42
TARGET_COL = "Calories"
...
def main():
    ...
    try:
        ...
        # Apply log1p to the target (without normalization by Duration)
        target_series_log = target_series_original.log1p()
        
        logger.info(f"Applied log1p transformation to target")
    ...
    test_preds_sum = np.zeros(len(test_pd_ce))
    models = []
    ...
    try:
        for fold, (train_idx, valid_idx) in enumerate(cv_indices_for_training):
            ...
            fold_test_preds = model.predict(test_pd_ce)

            # Store predictions
            oof_preds[valid_idx] = fold_oof_preds
            test_preds_sum += fold_test_preds
            
            logger.info(f"Fold {fold + 1} prediction complete. Best Iteration: {best_iteration}")
            logger.info("-" * 50)
    ...
    try:
        ...
        if exp_mask.sum() > 0:
            ...
            experiments_df.loc[idx_to_update, 'why'] = 'Test tuned LightGBM parameters without target normalization.'
            experiments_df.loc[idx_to_update, 'what'] = 'Train LightGBM with specific parameters (max_depth=10, learning_rate=0.02, etc.) using count encoding.'
            experiments_df.loc[idx_to_update, 'how'] = 'Apply count encoding using TEST data, train LightGBM with tuned parameters, KFold(7), log1p target (no normalization).'
            ...
            experiments_df.loc[idx_to_update, 'new_feature'] = 'Tuned LightGBM parameters'
            logger.info(f"Updated existing experiment '{EXPERIMENT_NAME}' with CV score: {overall_cv_score:.5f}")
             ...
             new_row = pd.DataFrame([{
                 ...
                 'why': 'Test tuned LightGBM parameters without target normalization.',
                 'what': 'Train LightGBM with specific parameters (max_depth=10, learning_rate=0.02, etc.) using count encoding.',
                 'how': 'Apply count encoding using TEST data, train LightGBM with tuned parameters, KFold(7), log1p target (no normalization).',
                 'cv_score': overall_cv_score,
                 'lb_score': None,
                 'status': 'done',
                 'experiment_type': 'single_model',
                 'base_experiment': 'exp_k_lgbm_normalized_target.py',
                 'new_feature': 'Tuned LightGBM parameters'
             }])
