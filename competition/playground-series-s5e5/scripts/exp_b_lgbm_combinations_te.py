...
Corresponds to experiment 'exp_b_lgbm_combinations_te.py' in experiments.csv.
This version uses 100% of the training data for S5E5: Calorie Prediction.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_b_lgbm_combinations_te.py" # Updated to match script filename
N_FOLDS = 7
SEED = 42
...
def main():
    ...
    try:
        ...
        # Use 100% of training data (no sampling)
        ...
        # Store test IDs
        test_ids = df_test['id']
        train_len = len(df_train) # Keep track of train length
    ...
    try:
        ...
        if exp_mask.sum() > 0: # Changed from == 1 to > 0 to handle potential multiple existing rows defensively
            ...
            experiments_df.loc[idx_to_update, 'why'] = 'Adapt script for S5E5, full data, log1p target transform.' # Update reason
            experiments_df.loc[idx_to_update, 'what'] = 'Train LGBM on log1p(Calories) with L1/RMSE metric.' # Update what
            experiments_df.loc[idx_to_update, 'how'] = 'Load 100% data, log1p target, FE adapted, KFold(7) LGBM (L1/RMSE on log), expm1 preds, final CV RMSLE.' # Update how
            logger.info(f"Updated existing experiment '{EXPERIMENT_NAME}' with CV score: {overall_cv_score:.5f} and status: done")
             ...
             new_row = pd.DataFrame([{
                 ...
                 'why': 'Adapt script for S5E5, full data, log1p target transform.',
                 'what': 'Train LGBM on log1p(Calories) with L1/RMSE metric.',
                 'how': 'Load 100% data, log1p target, FE adapted, KFold(7) LGBM (L1/RMSE on log), expm1 preds, final CV RMSLE.',
