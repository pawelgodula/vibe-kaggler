# Script for Experiment I: LightGBM with Count Encoding on Categorized Features
# MODIFIED for Playground Series S5E5: Predict Calorie Expenditure

"""
Extended Description:
This script is based on Experiment B but makes two key changes:
1. Treats all numerical features as categorical by converting them to strings
2. Uses Count Encoding instead of Target Encoding
...
- Out-of-fold Count Encoding using the `apply_count_encoding` utility
...
Corresponds to experiment 'exp_i_lgbm_count_encoding.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_i_lgbm_count_encoding.py"
N_FOLDS = 7
SEED = 42
TARGET_COL = "Calories"
...
def main():
    ...
    try:
        ...
        train_df_ce, test_df_ce = apply_count_encoding(
            ...
            count_on='train',  # Use only train data for counts
    ...
    try:
        ...
        if exp_mask.sum() > 0:
            ...
            experiments_df.loc[idx_to_update, 'why'] = 'Test count encoding and categorical treatment of numerical features.'
            experiments_df.loc[idx_to_update, 'what'] = 'Train LGBM using count encoding on categorical features, including discretized numerical features.'
            experiments_df.loc[idx_to_update, 'how'] = 'Convert numeric features to categorical bins, apply count encoding, KFold(7) LGBM, log1p target, expm1 preds.'
            experiments_df.loc[idx_to_update, 'experiment_type'] = 'single_model'
            experiments_df.loc[idx_to_update, 'base_experiment'] = 'exp_b_lgbm_combinations_te.py'
            experiments_df.loc[idx_to_update, 'new_feature'] = 'Count encoding + numerical features as categorical'
            logger.info(f"Updated existing experiment '{EXPERIMENT_NAME}' with CV score: {overall_cv_score:.5f}")
             ...
             new_row = pd.DataFrame([{
                 ...
                 'why': 'Test count encoding and categorical treatment of numerical features.',
                 'what': 'Train LGBM using count encoding on categorical features, including discretized numerical features.',
                 'how': 'Convert numeric features to categorical bins, apply count encoding, KFold(7) LGBM, log1p target, expm1 preds.',
                 'cv_score': overall_cv_score,
                 'lb_score': None,
                 'status': 'done',
                 'experiment_type': 'single_model',
                 'base_experiment': 'exp_b_lgbm_combinations_te.py',
                 'new_feature': 'Count encoding + numerical features as categorical'
             }])
