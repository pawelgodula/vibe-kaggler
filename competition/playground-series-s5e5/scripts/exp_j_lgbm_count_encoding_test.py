# Script for Experiment J: LightGBM with Count Encoding using Test Data 
# MODIFIED for Playground Series S5E5: Predict Calorie Expenditure

"""
Extended Description:
This script is based on Experiment I (LightGBM with Count Encoding) but makes two key changes:
1. Uses the test data for count encoding (frequency calculation) instead of training data
2. Adds feature importance visualization, saving the plot to the reports directory
...
Corresponds to experiment 'exp_j_lgbm_count_encoding_test.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_j_lgbm_count_encoding_test.py"
N_FOLDS = 7
SEED = 42
TARGET_COL = "Calories"
...
def main():
    ...
    try:
        ...
        train_df_ce, test_df_ce = apply_count_encoding(
            ...
            count_on='test',  # Use TEST data for counts (key change from exp_i)
        ...
        logger.error(f"Error during CE or CV setup: {e}", exc_info=True)
        sys.exit(1)
    ...
    try:
        ...
        if exp_mask.sum() > 0:
            ...
            experiments_df.loc[idx_to_update, 'why'] = 'Test count encoding based on test set frequencies and visualize feature importance.'
            experiments_df.loc[idx_to_update, 'what'] = 'Train LGBM using count encoding on categorical features with test set as encoding source.'
            experiments_df.loc[idx_to_update, 'how'] = 'Convert numeric features to categorical bins, apply count encoding using TEST data, KFold(7) LGBM, generate feature importance.'
            experiments_df.loc[idx_to_update, 'experiment_type'] = 'single_model'
            experiments_df.loc[idx_to_update, 'base_experiment'] = 'exp_i_lgbm_count_encoding.py'
            experiments_df.loc[idx_to_update, 'new_feature'] = 'Count encoding from test data + feature importance visualization'
            logger.info(f"Updated existing experiment '{EXPERIMENT_NAME}' with CV score: {overall_cv_score:.5f}")
             ...
             new_row = pd.DataFrame([{
                 ...
                 'why': 'Test count encoding based on test set frequencies and visualize feature importance.',
                 'what': 'Train LGBM using count encoding on categorical features with test set as encoding source.',
                 'how': 'Convert numeric features to categorical bins, apply count encoding using TEST data, KFold(7) LGBM, generate feature importance.',
                 'cv_score': overall_cv_score,
                 'lb_score': None,
                 'status': 'done',
                 'experiment_type': 'single_model',
                 'base_experiment': 'exp_i_lgbm_count_encoding.py',
                 'new_feature': 'Count encoding from test data + feature importance visualization'
             }])
