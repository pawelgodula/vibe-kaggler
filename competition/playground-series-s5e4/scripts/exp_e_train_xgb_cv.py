# Script for Experiment E: Train XGBoost using all CV folds.

"""
Extended Description:
This script runs the fourth baseline experiment: training an XGBoost model 
using all pre-computed cross-validation folds and the same numeric features 
as Experiment C. It utilizes the `train_and_cv` utility, calculates the 
overall CV score, saves artifacts, and updates the `experiments.csv` tracker.

Corresponds to experiment 'exp_e_train_xgb_cv.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_e_train_xgb_cv.py" # For updating experiments.csv
N_FOLDS = 5 
SEED = 42
TARGET_COL = "Listening_Time_minutes"

# Paths (Update output names)
...
MODEL_LIST_OUTPUT_PATH = MODEL_OUTPUT_DIR / f"{EXPERIMENT_NAME.replace('.py', '')}_models.joblib" # Changed extension if needed, joblib fine
...
# Features (Same numeric as Exp C)
NUMERIC_FEATURES = [
...
# XGBoost Model Parameters (Example defaults)
XGB_PARAMS = {
    'objective': 'reg:squarederror', 
    'eval_metric': 'rmse', 
    'eta': 0.05, # learning_rate alias
    'max_depth': 6,
    'subsample': 0.8,
...
# XGBoost Fit Parameters (Using n_estimators like concept)
XGB_FIT_PARAMS = {
    'num_boost_round': 1000, # Corresponds to n_estimators
    'early_stopping_rounds': 100,
    'verbose_eval': 100, # Log evaluation results
}

def main():
    """Main function to run the XGBoost CV training experiment."""
    ...
    # (Same NaN filling for numeric features as Exp C)
    ...
    try:
        oof_preds, test_preds_avg, models = train_and_cv(
            ...
            model_type='xgb', # Changed model type
            cv_indices=cv_indices,
            model_params=XGB_PARAMS, # Use XGBoost params
            fit_params=XGB_FIT_PARAMS, # Use XGBoost fit params
            cat_features=None,
            verbose=True 
        )
