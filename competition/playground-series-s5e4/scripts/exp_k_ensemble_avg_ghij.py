# Script for Experiment K: Averaging Ensemble of Experiments G, H, I, J

"""
Extended Description:
This script creates a simple average ensemble of the predictions generated by:
- exp_g_xgb_combinations_te.py (XGBoost, Full Data, TE)
- exp_h_lgbm_combinations_te.py (LGBM L1, Full Data, TE)
- exp_i_lgbm_l2_combinations_te.py (LGBM L2, Full Data, TE)
- exp_j_lgbm_l2_combinations_ce.py (LGBM L2, Full Data, CE)
...
Corresponds to experiment 'exp_k_ensemble_avg_ghij.py' in experiments.csv.
"""
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_k_ensemble_avg_ghij.py"
...
def main():
    ...
    # --- Average Predictions ---
    logger.info("Averaging OOF and Test predictions...")
    avg_oof_preds = np.mean(np.array(oof_preds_list), axis=0)
    avg_test_preds = np.mean(np.array(test_preds_list), axis=0)
    logger.info(f"Averaged OOF shape: {avg_oof_preds.shape}, Averaged Test shape: {avg_test_preds.shape}")
    ...
    try:
        ...
        why_desc = f"Average Ensemble of Exp {base_exp_names_str}"
        what_desc = f"Average OOF/Test predictions from Exp {base_exp_names_str}"
        how_desc = f"Load .npy files from {base_exp_names_str}, calculate np.mean, save results."
