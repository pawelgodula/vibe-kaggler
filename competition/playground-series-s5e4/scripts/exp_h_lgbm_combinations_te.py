...
# --- Configuration ---
EXPERIMENT_NAME = "exp_h_lgbm_combinations_te.py" # Updated experiment name
N_FOLDS = 7
SEED = 42
...
LGBM_PARAMS = {
    'objective': 'regression_l1', # MAE, often robust (like exp_d)
...
def main():
    ...
    try:
        ...
        if exp_mask.sum() == 1:
            # Should not exist, so add new row
            logger.warning(f"Experiment '{EXPERIMENT_NAME}' already exists in tracker? This should be a new experiment.")
            # Update existing row (less ideal but handles potential reruns)
            ...
            logger.info(f"Updated existing experiment '{EXPERIMENT_NAME}' with CV score: {overall_cv_score:.5f} and status: done")
             ...
             new_row = pd.DataFrame([{
                 ...
                 'why': 'Adapt Exp G (XGB) to LGBM with full data',
                 'what': 'Train LGBM with extensive FE including N-way label encoded combinations and TE',
                 'how': 'Load ext data, clip, map, poly/agg/N-way(label) feats, OOF TE, KFold(7) LGBM training.',
