# Script for Experiment G: XGBoost with Combination Features and Target Encoding

"""
Extended Description:
This script replicates the logic from a Kaggle notebook, implementing a pipeline
that includes:
...
- Training an XGBoost model using KFold cross-validation (7 folds).
- Incorporating an XGBoost learning rate scheduler callback.
...
Corresponds to experiment 'exp_g_xgb_combinations_te.py' in experiments.csv.
"""
...
try:
    ...
    from utils._train_xgb import _train_xgb # Import the utility
...
# --- Configuration ---
EXPERIMENT_NAME = "exp_g_xgb_combinations_te.py"
N_FOLDS = 7
SEED = 42
...
XGB_PARAMS = {
    ...
    'seed': SEED,
    'max_depth': 19,
    'learning_rate': 0.03, # Initial LR, will be modified by callback
    'min_child_weight': 50,
    'reg_alpha': 5,
    'reg_lambda': 1,
    'subsample': 0.85,
    ...
    'tree_method': 'hist', # Use hist for faster training, consider 'gpu_hist' if GPU available and configured
    # 'device': 'cuda' # Enable this if using gpu_hist and GPU is available
}

# Learning Rate Schedule Callback
def lr_decay(epoch, learning_rate=0.03, decay_start_epoch=115, decay_rate=0.1):
    # Replicates the script's logic: 0.03 until epoch 115, then 0.01 (approx 0.03 * 0.1 isn't 0.01, let's use fixed values)
    if epoch < decay_start_epoch:
        return 0.05 # Using 0.05 as in the script
    else:
        return 0.01 # Using 0.01 as in the script

xgb_callbacks = [xgb.callback.LearningRateScheduler(lr_decay)]

XGB_FIT_PARAMS = {
    'num_boost_round': 1000000, # A large number, rely on early stopping
    'early_stopping_rounds': 30,
    'verbose_eval': 500, # Passed to xgb.train
    'callbacks': xgb_callbacks # Add callbacks here for the utility
}
...
def main():
    ...
    try:
        ...
        # --- Store Original Train Length for Splitting ---
        train_len = len(df_train) # Keep track of *full* train length

        # --- NO SAMPLING: Use full training data (Commented out sampling) ---
        # sample_fraction = 0.1 # Use 10% of the training data
        # df_train = df_train.sample(fraction=sample_fraction, shuffle=True, seed=SEED)
        # logger.info(f"Sampled {sample_fraction*100:.0f}% of training data. New train shape: {df_train.shape}")

        # Store test IDs
        test_ids = df_test['id']
        # train_len = len(df_train) # Moved calculation above sampling block
        ...
        # --- Deduplication ---
        initial_rows = len(df)
        # df = df.unique(keep='first') # <<< COMMENTED OUT: This was removing test rows if they were duplicates of train/external rows
    ...
    try:
        ...
        # Convert Polars DFs to Pandas for XGBoost
        logger.info("Converting DataFrames to Pandas for XGBoost...")
    ...
    try:
        for fold, (train_idx, valid_idx) in enumerate(cv_indices_for_training):
            ...
            logger.info(f"Training XGBoost fold {fold + 1} using _train_xgb utility...")
            model, fold_oof_preds, fold_test_preds = _train_xgb(
                X_train=X_train_fold,
                y_train=y_train_fold.values, # Pass NumPy array
                X_valid=X_valid_fold,
                y_valid=y_valid_fold.values, # Pass NumPy array
                X_test=test_pd_te,       # Pass the full TE test dataframe
                model_params=XGB_PARAMS,
                fit_params=XGB_FIT_PARAMS, # Contains callbacks, early stopping etc.
            ...
            logger.info(f"Fold {fold + 1} prediction complete.")
            logger.info("-" * 50)
    ...
    try:
        ...
        if exp_mask.sum() == 1:
            ...
            logger.warning(f"Experiment '{EXPERIMENT_NAME}' not found or found multiple times in tracker. Adding new row.")
            # Add as new row if not found (ensure columns match)
            new_row = pd.DataFrame([{
                'script_file_name': EXPERIMENT_NAME,
                'why': 'Replicate Kaggle notebook (XGB+Combinations+TE)',
                'what': 'Train XGBoost with extensive FE including N-way label encoded combinations and TE',
                'how': 'Load ext data, clip, map, poly/agg/N-way(label) feats, OOF TE, KFold(7) XGBoost training with LR decay.',
